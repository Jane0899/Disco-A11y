...
Current fixes:
- **MelonLoaderProxy**: the July 2026 Windows update (KB5101650) makes the loader resolve `version.dll` from System32 even when a copy sits in the game folder — the classic MelonLoader proxy silently never loads and the game starts unmodded (no `Latest.log` written at all). `version.dll` is *not* a KnownDLL (checked via `NtOpenSection` against `\KnownDlls`); the exact loader mechanism is unconfirmed, but the behavior was verified live: with `version.dll` the game loaded `C:\Windows\System32\version.dll`, renamed to `winmm.dll` the bootstrap came up normally. The fix renames `version.dll` → `winmm.dll` (or retires it as `version.dll.disabled` when a `winmm.dll` already exists, so the proxy can never load twice). `MelonLoaderInstaller.IsInstalled` accepts either proxy name.
