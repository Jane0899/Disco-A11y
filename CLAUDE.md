...
### Disco Doctor (repair catalog, tools/Shared/ModFixes.cs)
One shared, modular catalog of known installation repairs (`ModFixCatalog`), compiled into **both** the installer and the configurator (`<Compile Include="..\Shared\ModFixes.cs">`). The installer runs it automatically on **every** install, right after the MelonLoader step — both preventively (a fresh MelonLoader zip ships a `version.dll` the fix immediately renames) and to heal existing installs. The configurator exposes it to users as **"Disco Doctor"**: a button on the main form opens a repair window (single-column list of known problems with their state, "Repair now", Tolk-spoken status), plus unattended CLI: `DiscoElysiumModConfigurator.exe --cli [gamePath] --repair`. Fixes are engine-only and carry no text; each tool localizes them via a `FixName_<Id>` string. Adding a future fix = one `ModFix` subclass + one catalog entry + a `FixName_<Id>` string in both `Strings.cs` files.

Current fixes:
- **MelonLoaderProxy**: since the July 2026 Windows update (KB5101650), **dynamic** loads of `version.dll` (`LoadLibrary` by name, including delay-load imports) resolve to System32 even when a copy sits in the application directory — verified with a probe exe placed in the game folder (`LoadLibraryExW("version.dll")` → System32; `winmm.dll` → local). **Static** imports still resolve app-dir-first (a Unity 2019 game's statically-importing UnityPlayer loaded its local `version.dll` fine on the same updated machine). Disco's Unity 2020.3 `UnityPlayer.dll` loads `version.dll` dynamically, so its MelonLoader proxy silently never loads and the game starts unmodded (no `Latest.log` written at all); games whose players import it statically are unaffected. `version.dll` is *not* a KnownDLL (checked via `NtOpenSection` against `\KnownDlls`). The fix renames `version.dll` → `winmm.dll` (dynamically still app-dir-resolved; or retires it as `version.dll.disabled` when a `winmm.dll` already exists, so the proxy can never load twice). `MelonLoaderInstaller.IsInstalled` accepts either proxy name. Should a future update give `winmm.dll` the same treatment, add the next proxy name as another catalog fix.
